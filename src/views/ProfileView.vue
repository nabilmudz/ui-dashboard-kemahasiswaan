<template>
  <div class="p-6 sm:p-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-1 bg-white border border-gray-150 rounded-2xl shadow-sm p-6 text-center flex flex-col items-center">
        <img 
        ...
        <span class="inline-block mt-3 px-3 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded-full uppercase tracking-wider">
          {{ authStore.user?.role }}
        </span>
        ...
        <div class="w-full flex flex-col gap-2 text-left text-xs">
          ...
          <div class="flex justify-between">
            <span class="text-gray-400 font-medium">SSO Provider</span>
            <span class="text-gray-800 font-semibold">farraskuy.com</span>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 flex flex-col gap-6">
        <div class="bg-white border border-gray-150 rounded-2xl shadow-sm p-6 flex flex-col">
          <h4 class="text-sm font-semibold text-gray-800 mb-4 border-b border-gray-100 pb-2">Informasi Token JWT</h4>
          
          <div class="flex flex-col gap-4 text-xs">
            <div class="flex flex-col bg-gray-50 p-3 rounded-lg border border-gray-100 font-mono break-all leading-relaxed">
...
const avatarUrl = computed(() => {
  ...
  if (foto === 'default.png' || foto.includes('default')) {
    return 'https://ui-avatars.com/api/?name=' + encodeURIComponent(authStore.user?.nama || 'User') + '&background=ffeeaf&color=4338ca&bold=true';
  }
  return foto;
});
</script>
