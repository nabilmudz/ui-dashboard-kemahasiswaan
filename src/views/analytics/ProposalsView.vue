<template>
  <div class="bg-white rounded-2xl border border-gray-100/70 p-6 shadow-none">
    ...
    <div v-else class="space-y-4">
      ...
      <div class="overflow-x-auto rounded-xl">
        <table class="w-full text-left border-collapse text-xs">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-for="prop in paginatedPendingProposals" :key="prop.submissionId" class="hover:bg-gray-50/50 transition-colors">
              <td class="py-3 px-4">
      ...
      <!-- Pagination Controls -->
      <div v-if="filteredPendingProposals.length > 0" class="flex justify-between items-center mt-4 pt-4 border-t border-gray-100 flex-wrap gap-3 text-xs select-none">
        <div class="flex items-center gap-2">
          ...
          <span class="text-gray-400 font-medium ml-1">
            Menampilkan {{ startIndex + 1 }}-{{ Math.min(endIndex, filteredPendingProposals.length) }} dari {{ filteredPendingProposals.length }} data
          </span>
        </div>
...
const totalPages = computed(() => {
  return Math.ceil(filteredPendingProposals.value.length / perPage.value) || 1;
});
...
const paginatedPendingProposals = computed(() => {
  return filteredPendingProposals.value.slice(startIndex.value, endIndex.value);
});

// Reset page on filter changes
watch([selectedAppFilter, perPage], () => {
  currentPage.value = 1;
});
