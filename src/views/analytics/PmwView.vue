    ...
    <div v-else class="space-y-6">
      ...
      <div v-if="authStore.user?.role === 'WD3'" class="space-y-6">
        ...
        <div v-if="pmwData.data" class="space-y-3">
          <h4 class="text-xs font-bold text-gray-800 uppercase tracking-wide">Sebaran Distribusi Bantuan Dana</h4>
          <div class="overflow-x-auto rounded-xl border border-gray-100/70">
            <table class="w-full text-left border-collapse text-xs">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="item in paginatedPmwData" :key="item.jurusan" class="hover:bg-gray-50/50">
          ...
          <!-- Pagination Controls -->
          <div v-if="pmwData.data && pmwData.data.length > 0" class="flex justify-between items-center mt-4 pt-4 border-t border-gray-100 flex-wrap gap-3 text-xs select-none">
            <div class="flex items-center gap-2">
              ...
              <span class="text-gray-400 font-medium ml-1">
                Menampilkan {{ startIndex + 1 }}-{{ Math.min(endIndex, pmwData.data.length) }} dari {{ pmwData.data.length }} data
              </span>
            </div>
...
const paginatedPmwData = computed(() => {
  const list = pmwData.value.data || [];
...
async function loadPmwData() {
  ...
  if (authStore.user?.role === 'WD3') {
    ...
    promises.push(
      api.get('/dashboard/analytics/pmw/dana-usulan-vs-rekomendasi')
        .then(res => {
